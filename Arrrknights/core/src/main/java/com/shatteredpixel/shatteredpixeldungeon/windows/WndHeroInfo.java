...
public class WndHeroInfo extends WndTabbed {
	...
	public WndHeroInfo( HeroClass cl ){
		...
		switch (cl){
			...
			case RABBIT:
				...
				break;
			case MIDORI:
				tabIcon = new ItemSprite(ItemSpriteSheet.R4C, null);
				break;
		}
	...
	private static class TalentInfoTab extends Component {
		...
		public TalentInfoTab( HeroClass cls ){
			super();
			title = PixelScene.renderTextBlock(Messages.titleCase(Messages.get(WndHero.class, "talents")), 9);//change from budding
			title.hardlight(TITLE_COLOR);
			add(title);
