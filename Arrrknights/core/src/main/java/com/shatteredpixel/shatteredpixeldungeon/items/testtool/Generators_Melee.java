...
public class Generators_Melee extends Generators {
    {
    ...
    private Class<? extends Weapon.Enchantment> generateEnchant(int category, int id) {
        if (category == 1) switch (id) {
            ...
            case 6:
                return Penetrate.class;
            case 7:
                return Overeating.class;
            default:
                return null;
        }
    ...
    private class SettingsWindow extends Window {
        ...
        private void createWeaponImage(Class<? extends Weapon>[] all) {
            ...
            for (int i = 0; i < length; ++i) {
                ...
                int half=(length+1)/2;
                left = (WIDTH - BTN_SIZE * half)/2f;
                for (int i=0;i<2;i++){
                    for (int j=0;j<half && i*half+j<length;++j){
                        final int k=i*half+j;
                        IconButton btn=new IconButton(){
                            @Override
                            protected void onClick() {
                                weapon_id = Math.min(maxSlots(tier) - 1, k);
                                updateSelectedWeaponText();
                                super.onClick();
                            }
                        };
                        Image im = new Image(Assets.Sprites.ITEMS);
                        im.frame(ItemSpriteSheet.film.get(Objects.requireNonNull(Reflection.newInstance(all[i*half+j])).image));
                        im.scale.set(0.5f);
                        btn.icon(im);
                        btn.setRect(left + (placed-i*half) * BTN_SIZE, top+i*BTN_SIZE, BTN_SIZE, BTN_SIZE);
                        add(btn);
                        placed++;
                        iconButtons.add(btn);
                    }
                ...
                left = (WIDTH - BTN_SIZE * third)/2f;
                for (int i=0;i<3;i++){
                    for (int j=0;j<third && i*third+j<length;++j){
                        final int k=i*third+j;
                        IconButton btn=new IconButton(){
                            @Override
                            protected void onClick() {
                                weapon_id = Math.min(maxSlots(tier) - 1, k);
                        ...
                        Image im = new Image(Assets.Sprites.ITEMS);
                        im.frame(ItemSpriteSheet.film.get(Objects.requireNonNull(Reflection.newInstance(all[i*third+j])).image));
                        im.scale.set(0.5f);
                        btn.icon(im);
                        btn.setRect(left + (placed-i*third) * BTN_SIZE, top+i*BTN_SIZE, BTN_SIZE, BTN_SIZE);
                        add(btn);
                        placed++;
                        iconButtons.add(btn);
                    }
        ...
        private void layout() {
            o_tier.setRect(0, GAP, WIDTH, 24);
            t_selectedWeapon.setPos(0, GAP * 4 + o_tier.bottom() + BTN_SIZE*3);
