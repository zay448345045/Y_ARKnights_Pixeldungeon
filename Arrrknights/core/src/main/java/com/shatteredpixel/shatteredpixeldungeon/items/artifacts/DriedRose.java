		...
		public boolean act() {
			...
			if (ghost != null){
				...
				if (ghost.HP < ghost.HT && (lock == null || lock.regenOn())) {
					partialCharge += (ghost.HT / 1000f) * RingOfEnergy.artifactChargeMultiplier(target) * LFPChargeMultiplier();
					updateQuickslot();
			...
			if (charge < chargeCap && !cursed && (lock == null || lock.regenOn())) {
				...
				partialCharge += (1/5f * RingOfEnergy.artifactChargeMultiplier(target) * LFPChargeMultiplier());
				if (partialCharge > 1){
					charge++;
					partialCharge--;
					if (charge == chargeCap){
