			...
			if (activeBuff == null){
				if (!isEquipped(hero)&& !hero.hasTalent(Talent.LIGHT_CLOAK)) GLog.i( Messages.get(Artifact.class, "need_to_equip") );
					...
					if (hero.hasTalent(Talent.LIGHT_CLOAK) && hero.buff(Talent.LightClockCooldown.class) == null) {
						...
						Buff.affect(hero, Talent.LightClockCooldown.class, 100);
						hero.spend( 1f );
		...
		public boolean act() {
			if (charge < chargeCap) {
				...
				if (activeBuff == null && (lock == null || lock.regenOn())  && !(Dungeon.depth >= 26 && Dungeon.depth <= 30)) {
					...
					turnsToCharge /= RingOfEnergy.artifactChargeMultiplier(target)*LFPChargeMultiplier();
	...
	public class cloakStealth extends ArtifactBuff{
		...
		public boolean attachTo( Char target ) {
			if (super.attachTo( target )) {
				target.invisible++;
				if (target instanceof Hero && ((Hero) target).subClassSet.contains(HeroSubClass.ASSASSIN)){
