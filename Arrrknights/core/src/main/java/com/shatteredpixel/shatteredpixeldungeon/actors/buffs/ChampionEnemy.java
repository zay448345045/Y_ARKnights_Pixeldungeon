...
public abstract class ChampionEnemy extends Buff {
	...
	public static void toChampion(Mob m){
		...
		if (((Dungeon.mobsToChampion <= 0 && Dungeon.isChallenged(Challenges.CHAMPION_ENEMIES))|| Statistics.victoryLapRounds>0)
			&& m.buff(buffCls)==null) {
	...
	public static void toHonor(Mob m){
		Class<?extends ChampionEnemy> buffCls;
		switch (Random.Int(7)){
		...
		if (((Dungeon.mobsToHonor <= 0 && Dungeon.isSPChallenged(SPChallenges.HONOR) && buffCls != null) || Statistics.victoryLapRounds>0)
			&& m.buff(buffCls)==null) {
			Buff.affect(m, buffCls);
