...
public class TrapPlacer extends Generators {
    {
    ...
    static {
        ...
        trapLib.put(56, OriginiumTrap.class);
        trapLib.put(57, HallucinationTrap.class);
        trapLib.put(58, OblivionTrap.class);
    }
    ...
    private class SettingsWindow extends Window{
        ...
        public SettingsWindow(){
            ...
            for (int k=56;k<59;++k){
                ...
                IconButton btn=new IconButton(){
                    ...
                    public void onClick(){
                        ...
                        row = 7;
                        column = num-56;
                        trapButtons.get(num).icon().color(0xFFFF44);
                        updateText();
                    }
                };
                btn.setRect(left + BTN_SIZE * (k-56), top + BTN_SIZE * 7, BTN_SIZE, BTN_SIZE);
                add(btn);
                trapButtons.add(btn);
            }
            refreshImage();
            ...
            cb.setRect(1, top + 8 * BTN_SIZE + 4, WIDTH/2f, 16);
            add(cb);

            selected.setPos(WIDTH/2f + 6, top + 8 * BTN_SIZE + 4 + (cb.height()-7f)/2f);
        ...
        private String statDesc() {
            if(trap == null || trap instanceof EmptyTrap) return M.L(this, "null_trap");
        ...
        private void refreshImage(){
            for(int i = 0, size = trapButtons.size(); i < size; ++i){
                ...
                if(trapLib.get(i)==null && !enableInvalidImage){
                    ti=new Image(Assets.Environment.TERRAIN_FEATURES, 256, 32*(i/COLS), 32, 32);
                }else {
                    if (i < 56) {
                        ti = new Image(Assets.Environment.TERRAIN_FEATURES, 32 * (i % COLS), 32 * (i / COLS), 32, 32);
                    } else {
                        ti = new Image(Assets.Environment.TERRAIN_FEATURES, 32 * (i - 56), 256, 32, 32);
                    }
                }
                ti.scale.set(0.5f);
                trapButtons.get(i).icon(ti);
            }
        }
    }
