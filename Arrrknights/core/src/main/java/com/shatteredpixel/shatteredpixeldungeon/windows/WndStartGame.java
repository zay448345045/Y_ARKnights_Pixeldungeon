	...
	private class HeroPane extends Component {
		...
		protected void createChildren() {
			...
			heroItem = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					TomorrowRogueNight.scene().addToFront(new WndMessage(Messages.get(cl, cl.name() + "_desc_item")));
				}
			};
			...
			heroLoadout = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					TomorrowRogueNight.scene().addToFront(new WndMessage(Messages.get(cl, cl.name() + "_desc_loadout")));
				}
			};
			...
			heroMisc = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					TomorrowRogueNight.scene().addToFront(new WndMessage(Messages.get(cl, cl.name() + "_desc_misc")));
				}
			};
				...
				if (cl != null) {
					...
					switch(cl){
						...
						case ROSECAT:
							...
							break;
						case NEARL:
							heroItem.icon(new ItemSprite(ItemSpriteSheet.ARTIFACT_HOLDER, null));
						...
						case RABBIT:
							...
							break;
						case MIDORI:
							heroItem.icon(new ItemSprite(ItemSpriteSheet.R4C, null));
							heroLoadout.icon(new ItemSprite(ItemSpriteSheet.WEAPON_HOLDER, null));
							heroMisc.icon(new Image(Assets.Environment.TILES_SEWERS, 112, 96, 16, 16 ));
							break;
					}
